...
$spents = OperationData::getAllByDate($_GET["date"]);
}else{
$spents = OperationData::getAllByDateAndCategoryId($_GET["date"],$_GET["cat_id"]);
$category = CategoryData::getById($_GET["cat_id"]);
}
$money = 0;
 foreach($spents as $career){ $money += $career->q*$career->price; }?>
 <section class="content-header">
	<h1>Historial de ventas <small><?php 
...
<div class="panel panel-default">
  ...
  <a class='list-group-item' href="index.php?view=sellbydate&date=<?php echo $_GET["date"];?>"><?php if($category==null): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?>Todas</a>
<?php foreach(CategoryData::getAll() as $cat):?>
  <a class='list-group-item' href="index.php?view=sellbydate&date=<?php echo $_GET["date"];?>&cat_id=<?php echo $cat->id; ?>"><?php if( $category!=null && ($category->id==$cat->id)): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?><?php echo $cat->name; ?></a>
<?php endforeach; ?>
	</div>
</div>
</div>
